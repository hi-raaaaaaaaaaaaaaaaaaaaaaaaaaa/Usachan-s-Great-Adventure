...
namespace Game
{
    public partial class frmMain : Form
    {
		...
		private void frmMain_Load(object sender, EventArgs e)
        {
			...
			enemies.Add(new clsEnemy(Width / 2, 200, imgEnemy));
			enemies.Add(new clsEnemy(Width / 4, 200, imgEnemy));
			enemies.Add(new clsEnemy(Width / 6, 200, imgEnemy));
		...
		private void timer1_Tick(object sender, EventArgs e)
		{
			...
			{
				if (x + imgPlayer.Width >= enemies[k].posision.X && x <= enemies[k].posision.X + imgEnemy.Width && y + imgPlayer.Height >= enemies[k].posision.Y && y <= enemies[k].posision.Y + imgEnemy.Height) player.destroy(Width);
			}
			...
			for (int j = shots_en.Count - 1; j >= 0; j--)
			{
				...
				shot_en.move();
				if (shot_en.posision.X + imgShot_en.Width >= player.posision.X && shot_en.posision.X <= player.posision.X + imgPlayer.Width && shot_en.posision.Y + imgShot_en.Height >= player.posision.Y && shot_en.posision.Y <= player.posision.Y + imgPlayer.Height) player.destroy(Width);
